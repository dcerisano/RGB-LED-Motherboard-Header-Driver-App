...
# MAIN LOOP
  ...
  do 
     ...
     then
        ...
        curl -X GET 'http://praxis:8080?method=set_shader_req&shader=notifier&image=vrip.jpg' > /dev/null 2>&1 
     fi
     
     if [[ $line == *"standard3d"* ]]
     then
        aplay /usr/local/share/standard3d.wav &
        curl -X GET 'http://praxis:8080?method=set_shader_req&shader=notifier&image=s3d.jpg' > /dev/null 2>&1 
     fi
